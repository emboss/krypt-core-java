...
/*
 * Huge Credits to jruby-ossl for this implementatioln
 */

package org.jruby.krypt.crypto;
...
import org.jruby.ext.krypt.KryptImpl;
import org.jruby.ext.krypt.SimpleSecretKey;
...
public class Cipher extends RubyObject {
    ...
    public static void createCipher(Ruby runtime, RubyModule mKrypt) {
        RubyClass cCipher = mKrypt.defineClassUnder("Cipher", runtime.getObject(), CIPHER_ALLOCATOR);
        cCipher.defineAnnotatedMethods(Cipher.class);
        cCipher.defineAnnotatedMethods(CipherModule.class);
        RubyClass KryptError = mKrypt.getClass("KryptError");
        cCipher.defineClassUnder("CipherError", KryptError, KryptError.getAllocator());
    }

    @JRubyModule(name = "Krypt::Cipher")
    ...
    protected void init(IRubyObject[] args, boolean encrypt) {
        ...
        if (args.length > 0) {
            ...
            try {
                ...
                Errors.newDigestError(getRuntime(), "cannot get digest instance");
            }
            KryptImpl.KeyAndIv result = KryptImpl.EVP_BytesToKey(keyLen, ivLen, digest, iv, pass, 2048);
    ...
    javax.crypto.Cipher getCipher() {
        try {
            ...
            //Removes BC Dependency
            //TODO: Fix independent impl
            //try {
    ...
    public IRubyObject pkcs5_keyivgen(IRubyObject[] args) throws NoSuchAlgorithmException {
        ...
        final String algorithm = vdigest.isNil() ? "MD5" : ((Digest) vdigest).getName();
        //TODO: Create a KeyAndIV Impl
        MessageDigest digest = MessageDigest.getInstance(algorithm);//Digest.getDigest(algorithm, getRuntime());
        KryptImpl.KeyAndIv result = KryptImpl.EVP_BytesToKey(keyLen, ivLen, digest, salt, pass, iter);
        this.key = result.getKey();
    ...
    private void doInitialize() {
        ...
        try {
            ...
            throw Errors.newCipherError(getRuntime(), ike.getMessage() + ": possibly you need to install Java Cryptography Extension (JCE) Unlimited Strength Jurisdiction Policy Files for your JRE");
        } 
        catch (Exception e) {
            if (DEBUG) {
                e.printStackTrace();
    ...
    private static RaiseException newCipherError(Ruby runtime, String message) {
          return Errors.newError(runtime, "Krypt::Cipher::CipherError", message);
    }
}